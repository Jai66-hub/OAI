...
public:
  ...
  /** @enum SecurityHeaderType */
    ...
    /** @enum Sparehalfoctet */
    enum class Sparehalfoctet : uint8_t {
        spare
    }; 
    ...
    /** @brief code the spare half octet 
     *
     * This method code the SHO and add it to the data buffer
     *
     * @param[in] security  Spare half octet 
     * 
     * @param[out] data     buffer to add SHO
     * 
     * @return number of bytes added to the data buffer
     */
    static int codeSparehalfoctet(std::vector<uint8_t> &data,const Sparehalfoctet halfoctet);
    ...
    /** @brief decode Security header type from byte buffer
     *
     * @param[in] data buffer containing PD/EDP byte
     *
     * @return security header type value
     *
     * @throw std::invalid_argument   
     */    
    static SecurityHeaderType decodeSecurityHeaderType(const std::vector<uint8_t> &data);
   /** @brief decode spare half octet type from byte buffer
     *
     * @param[in] data buffer containing PD/EDP byte
     *
     * @return  spare half octet type
     *
     * @throw std::invalid_argument   
     */    
    
    static Sparehalfoctet decodeSparehalfoctet(const std::vector<uint8_t> &data);
    ...
    static Nas::SecurityHeaderType uint8_t_to_Securityheadertype(const uint8_t byte);
    static Nas::Sparehalfoctet uint8_t_to_Sparehalfoctet(const uint8_t byte);
